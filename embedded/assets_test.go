...
func TestUpdatedEmbedded(t *testing.T) {
	...
	for _, f := range embedded.AssetNames() {
		...
		if want, got := getHTTP(t, wantURL), getHTTP(t, gotURL); want != got {
			...
			t.Errorf(`Try running 'go-bindata -o embedded/assets.go -ignore \\.bower\.json -ignore bower_components/marked -ignore bower_components/core-scroll-header-panel/demos -ignore bower_components/core-tests -pkg embedded public/...'`)
		}
	}
}
